...
-- Autor: FACO Team
-- Fecha: 2025-06-19
-- Versión: 1.0.1 (Corregido para BigQuery)
...
BEGIN
  ...
  -- Valores por defecto
  SET p_fecha_inicio = IFNULL(p_fecha_inicio, CURRENT_DATE('America/Lima'));
  SET p_fecha_fin = IFNULL(p_fecha_fin, CURRENT_DATE('America/Lima'));
  ...
  VALUES (
    CURRENT_TIMESTAMP(),
    'stage_pagos',
    DATE(v_inicio_proceso),
    'INICIADO',
    0,
    0.0,
    CONCAT('Procesando pagos del período: ', v_periodo_analisis, '. Pagos detectados: ', CAST(v_pagos_detectados AS STRING)),
    JSON_OBJECT(
  ...
  BEGIN
    ...
    USING (
      ...
      pagos_con_gestion_atribuida AS (
        ...
        LEFT JOIN `BI_USA.bi_P3fV4dWNeMkN5RJMhV8e_stg_gestiones` ges
          ON CAST(p.cod_luna AS INT64) = ges.cod_luna  -- ✅ Conversión de tipos
      ...
      pagos_procesados AS (
        SELECT
          ...
          COALESCE(canal_origen, 'SIN_GESTION_PREVIA') AS canal_atribuido,
    ...
    VALUES (
      ...
      CONCAT(
        ...
        'Período: ', v_periodo_analisis, '. ',
        'Pagos detectados: ', CAST(v_pagos_detectados AS STRING), '. ',
        'Nuevos: ', CAST(v_registros_nuevos AS STRING), '. ',
        'Actualizados: ', CAST(v_registros_actualizados AS STRING)
      ),
      JSON_OBJECT(
        'fecha_inicio', CAST(p_fecha_inicio AS STRING),
        'fecha_fin', CAST(p_fecha_fin AS STRING),
        ...
        'duracion_proceso_segundos', TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), v_inicio_proceso, SECOND)
      )
    );

  EXCEPTION WHEN ERROR THEN
    -- Log de error en BigQuery
    INSERT INTO `BI_USA.pipeline_logs` (
      timestamp,
      stage_name,
      fecha_proceso,
    ...
    VALUES (
      ...
      0,
      TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), v_inicio_proceso, SECOND),
      CONCAT('Error en procesamiento del stage de pagos. Período: ', v_periodo_analisis),
      JSON_OBJECT(
        'fecha_inicio', CAST(p_fecha_inicio AS STRING),
        'fecha_fin', CAST(p_fecha_fin AS STRING),
        'modo_ejecucion', p_modo_ejecucion,
        'error_context', 'Error durante ejecución del MERGE'
      )
    );

    -- Re-lanzar el error
    RAISE USING MESSAGE = 'Error en stage_pagos pipeline';
  END;
