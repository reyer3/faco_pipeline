 ...
 */
  ...
  -- Asignación individual usando subconsultas escalares
  SET v_es_dia_apertura = (
    SELECT COUNT(*) > 0
    FROM `mibot-222814.BI_USA.bi_P3fV4dWNeMkN5RJMhV8e_dash_calendario_v5`
    WHERE FECHA_ASIGNACION = p_fecha_proceso
  );

  SET v_carteras_abriendo = (
    SELECT STRING_AGG(DISTINCT ARCHIVO, ', ')
    FROM `mibot-222814.BI_USA.bi_P3fV4dWNeMkN5RJMhV8e_dash_calendario_v5`
    WHERE FECHA_ASIGNACION = p_fecha_proceso
  );

  SET v_fechas_trandeuda = (
    SELECT STRING_AGG(DISTINCT CAST(FECHA_TRANDEUDA AS STRING), ', ')
    FROM `mibot-222814.BI_USA.bi_P3fV4dWNeMkN5RJMhV8e_dash_calendario_v5`
    WHERE FECHA_ASIGNACION = p_fecha_proceso
  );
  ...
  WHERE REGEXP_CONTAINS(archivo, CONCAT('TRAN_DEUDA_', fecha_ddmm))
